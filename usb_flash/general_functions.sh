...
# The function takes a one argument, which is the file that needs to be imported. The \
...
function generate_import_status_name() {
	...
	if [[ $# -eq 1 && -n "$1" && -f "$1" ]] ; then
		...
		# file name(with the extension, if any)
		#
	...
	if [[ $# -gt 0 ]] ; then
		...
		for VARIABLE in "$@" ; do
			...
			if [[ -n "${VARIABLE}" && -s "${VARIABLE}" && -r "${VARIABLE}" ]] ; then
				...
				# The below two blocks of text needs to be removed 

				# Using substring removal to get rid of the path to the file
				# This gives only the filname(with the extension, if any)
				# If input="/path/to/file.sh", importStatusName="file.sh"
				# If input="/path/to/file", importStatusName="file"
				#importStatusName="${VARIABLE##*/}"

				# Using substring removal to get rid of the extension, if any, and adding the string \
				# 'importStatus' to the end of the variable
				# If input="file.sh", importStatusName="file_importStatus"
				# If input="file", importStatusName="file_importStatus"
				#importStatusName="${importStatusName%.*}_importStatus"

				# Generating the import status name by giving the file name as input
