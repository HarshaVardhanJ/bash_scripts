...
# checked.
#
# Usage  : get_image_file__file_signature_check_method "/path/to/file"
# Output : The file's type and signature is returned.
function get_image_file__file_signature_check_method() {
...
# as output.
#
# Usage  : get_image_file__file_signature_match_check "/path/to/file"
# Output : /absolute/path/to/file
function get_image_file__file_signature_match_check() {
  ...
  # Signatures=("ISO file" "Zip file" "tar.gz file" "bzip2 file")
  ...
  if [[ $# -eq 1 ]] ; then
    ...
    if [[ -s "$1" && -r "$1" ]] ; then
      ...
      fileSignature="$(get_image_file__file_signature_check_method "$1")"


      # If the exit code of the previous command is not 1
      # that is, if the function reponsible for obtaining \
      # the file type and signature was successful
      if [[ $? -ne 1 ]] ; then
        # For a list of all signatures in the 'fileSignatureArray' array
        for Variable in "${fileSignatureArray[@]}" ; do
          # If the fileSignature value matches any of the permitted signatures \
          # in the array
          if [[ "${fileSignature}" =~ "${Variable}" ]] ; then
